...
def train(model, batch_x, batch_y, x_test, y_test, sess, training_iters=5000, test_step=10):
    train_writer = tf.summary.FileWriter('./train_v2', sess.graph)
    generation_error = []
    step = 1
    ...
    np.save('generation_v2.npy', np.array(generation_error))
    train_writer.close()
...
def save(sess):
    ...
    save_path = saver.save(sess, "./model_v2/model.ckpt")
...
if __name__ == "__main__":
    ...
    x_test, y_test = my_network.load_data_source(filename="./cc_v2/test.tfrecord")
